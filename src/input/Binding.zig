...
pub const Action = union(enum) {
    ...
    /// (ramon fork) Hide the FOCUSED split from the Agent Dashboard (macOS) —
    /// the keyboard equivalent of a tile's eye-slash Hide button, so you can
    /// declutter the dashboard from the agent's own split. Hide-only (idempotent
    /// — pressing it on an already-hidden split keeps it hidden; reveal from the
    /// dashboard's Show button). A hidden agent still auto-unhides the instant it
    /// rings / needs input. Surface-scoped, payload-less.
    hide_dashboard_split,

    /// (ramon fork) Spotlight the FOCUSED split in the Agent Dashboard (macOS): unhide
    /// it and float its tile to the very TOP of the dashboard (above every other tile,
    /// including queue sections and attention/waiting tiles) for
...
test "Binding hide_dashboard_split" {
    const testing = std.testing;
    const alloc = testing.allocator;

    // Parses bare name to the payload-less tag.
    const binding = try parseSingle("a=hide_dashboard_split");
    try testing.expect(binding.action == .hide_dashboard_split);
