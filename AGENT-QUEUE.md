...
- **Run the same template in parallel for different scopes.** A run's identity is its template
  **plus its resolved scope** (the env-param answers — e.g. project + milestone). Starting the
  same template again with the **same** scope is a no-op (it won't reset the live run); starting
  it with a **different** milestone/project opens a **second, parallel run** in its own tab with
  its own health bar. So two milestones run at once from one template — you don't need to
  duplicate the template file. (Because a same-scope re-start is a no-op, you can't rescope or
  re-cap a live run by re-starting it — use the tap-to-edit cap above for maxItems, or Abort +
  start fresh to change scope.)
