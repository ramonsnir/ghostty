...
- **Agent Dashboard** (fork-only, macOS, OFF by default; config `agent-dashboard` /
  ...
  **`toggle_dashboard_hide`** is the keyboard equivalent of a tile's eye-slash Hide button: a
  ...
  plumbing, wiring + tests.** **Preview geometry — FIT-TO-WIDTH + host-sourced cell size
  (always on):** `AgentMirrorPreview.geometry` scales each preview so the agent pane's OWN width
  fills the tile (`scale = container.width / naturalW` ⇒ `scaledW == container.width` for any
  column count: narrow panes scale up, wide ones down), bottom-anchored + height-clipped. Replaces
  a fixed 125-column reference scale that left narrower panes using only `cols/125` of the width
  (≈⅓ for ~40-col splits) and rescaled that fraction oddly on a dashboard resize. The backing
  factor cancels out of `scaledW`/`scaledH` (robust to backing mismatch). Cell size
  (`cell_width/height_px`) AND cols/rows all come from the HOST surface, NOT the mirror's own
  `surfaceSize` — the mirror's `cell_width_px` jitters ±1px (17↔16) as its frame re-rounds onto a
  sub-pixel boundary, which (when it fed the scale) caused a ~160 Hz "preview keeps switching font
  size" flicker, display-dependent (surfaced after moving the window between displays of different
  scale); the host's cell size is stable and breaks the loop. Wiring: `AgentPreviewTile.swift`
  (`geometry` + the body's host-first cell-size reads). Tests: `AgentMirrorGeometryTests`
  (`fitsWidthForEverySplitWidth`/`scaledWidthIsBackingIndependent`/`resizeKeepsFillingWidth`).
