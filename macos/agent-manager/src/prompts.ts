...
export const SUMMARIZER_BASE_PROMPT = [
  ...
  "  and shown to the user. Decide from ONE question: is the user being asked to ACT right",
  "  now? Judge the LIVE bottom-of-screen state, not scrollback.",
  "  Set `attention: false` when the bell is INCIDENTAL — the agent is NOT awaiting human",
  "  input. Incidental cases (ALL => false):",
  "    * a background task or workflow just STARTED (e.g. \"running in the background\");",
  "    * a long-running workflow / review / pipeline / test or build the user kicked off is",
  "      STILL PROGRESSING, or just finished a STAGE, and they are merely awaiting its",
  "      completion — no decision is being asked of them (e.g. \"5/6 review agents done\",",
  "      \"suite still running\", \"build in progress\"). The agent may even report `waiting`",
  "      here — it is waiting on its OWN sub-tasks, NOT on the human;",
  "    * a sub-step finished; a routine notification.",
  "  Keep the bell LOUD (set `attention: true`, or OMIT — both stay loud) ONLY when the live",
  "  bottom-of-screen state ACTUALLY awaits the user: a question / approval / choice / error",
  "  needing a decision, or a workflow that has STOPPED and now needs them. The system fails",
  "  open, so when you are GENUINELY unsure, leave it loud — but \"a workflow is running\" is",
  "  NOT unsure, it is incidental. When no bell-rang note is present, OMIT attention entirely.",
